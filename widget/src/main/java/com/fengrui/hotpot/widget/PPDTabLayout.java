...
package com.fengrui.hotpot.widget;
 ...
 *
 * <p>You can also add items to TabLayout in your layout through the use of {@link android.support.design.widget.TabItem}.
 ...
 *
 *     &lt;android.support.design.widget.android.support.design.widget.TabItem
 ...
 */
@ViewPager.DecorView
public class PPDTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link PPDTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link PPDTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    int mDivideLineWidth;
    int mDivideLineColor;
    int mDivideLinePadding;
    ...
    public PPDTabLayout(Context context) {
        this(context, null);
    }

    public PPDTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PPDTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        final int[] APPCOMPAT_CHECK_ATTRS = {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PPDTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout);

        mTabStrip.setSelectedIndicatorWeight(
                a.getFraction(R.styleable.PPDTabLayout_tabPPDIndicatorWeight, 1, 1, 1.0f));
        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDIndicatorHeight, 0));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.PPDTabLayout_tabPPDIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(R.styleable.PPDTabLayout_tabPPDTextAppearance,
                R.style.TextAppearance_Design_Tab);

        mTabTextSize = a.getDimensionPixelSize(
                    R.styleable.PPDTabLayout_tabPPDTextSize, 45);

        if (a.hasValue(R.styleable.PPDTabLayout_tabPPDTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.PPDTabLayout_tabPPDTextColor);
        }

        if (a.hasValue(R.styleable.PPDTabLayout_tabPPDSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.PPDTabLayout_tabPPDSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mDivideLineWidth = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDDivideLineWidth, 0);
        mDivideLineColor = a.getColor(R.styleable.PPDTabLayout_tabPPDDivideLineColor, 0xdcdcdc);
        mDivideLinePadding = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDDivideLinePadding, 0);

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.PPDTabLayout_tabPPDBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.PPDTabLayout_tabPPDContentStart, 0);
        mMode = a.getInt(R.styleable.PPDTabLayout_tabPPDMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.PPDTabLayout_tabPPDGravity, GRAVITY_FILL);
        a.recycle();
...
//    private void addTabFromItemView(@NonNull android.support.design.widget.TabItem item) {
...
//    }
    ...
    /**
     * Add a {@link PPDTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     *
    ...
    /**
     * Remove the given {@link PPDTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link PPDTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link PPDTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link PPDTabLayout} with a {@link ViewPager}.
     *
...
//        if (child instanceof android.support.design.widget.TabItem) {
//            addTabFromItemView((android.support.design.widget.TabItem) child);
//        } else {
//            throw new IllegalArgumentException("Only android.support.design.widget.TabItem instances can be added to TabLayout");
//        }
        throw new IllegalArgumentException("Don't support addView now");
    }
    ...
    class TabView extends LinearLayout implements OnLongClickListener {
        ...
        public TabView(Context context) {
            super(context);
            if(mDivideLineWidth > 0) {
                mRealLayout = new LinearLayout(getContext());
                setOrientation(HORIZONTAL);
//                addTabDivideLine();
                addView(mRealLayout, createLayoutParamsForTabs());
                mRealLayout.setClickable(false);
            } else {
                mRealLayout = this;
            }
            if (mTabBackgroundResId != 0) {
                ViewCompat.setBackground(
                        mRealLayout, AppCompatResources.getDrawable(context, mTabBackgroundResId));
            }
            ViewCompat.setPaddingRelative(mRealLayout, mTabPaddingStart, mTabPaddingTop,
                    mTabPaddingEnd, mTabPaddingBottom);
            mRealLayout.setGravity(Gravity.CENTER);
            mRealLayout.setOrientation(VERTICAL);
            setClickable(true);
            ViewCompat.setPointerIcon(mRealLayout,
                    PointerIconCompat.getSystemIcon(getContext(), PointerIconCompat.TYPE_HAND));
        }

        private void addTabDivideLine() {
            if(mDivideLineWidth > 0) {
                mDivideLine = new View(getContext());
                LayoutParams lp = new LayoutParams(mDivideLineWidth,
                        ViewGroup.LayoutParams.MATCH_PARENT);
                lp.topMargin = lp.bottomMargin = mDivideLinePadding;
                ViewCompat.setBackground(mDivideLine, new ColorDrawable(mDivideLineColor));
                addView(mDivideLine, 0, lp);
            }
        }
...
//                update();
        ...
        final void update() {
            ...
            if(mDivideLineWidth > 0 && mTab.getPosition() > 0) {
                addTabDivideLine();
            }
            if (custom != null) {
                ...
                if (mCustomView != null) {
                    mRealLayout.removeView(mCustomView);
                    mCustomView = null;
                }
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    ...
                    mRealLayout.addView(iconView, 0);
                    mIconView = iconView;
                }
                if (mTextView == null) {
                    TextView textView = (TextView) LayoutInflater.from(getContext())
                            .inflate(R.layout.design_layout_tab_text, this, false);
                    mRealLayout.addView(textView);
                    mTextView = textView;
    ...
    private class SlidingTabStrip extends LinearLayout {
        private float mSelectedIndicatorWeight;
        ...
        void setSelectedIndicatorWeight(float weight) {
            if(weight < 0 || weight > 1) {
                weight = 1.0f;
            }
            if(mSelectedIndicatorWeight != weight) {
                mSelectedIndicatorWeight = weight;
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
        ...
        private void updateIndicatorPosition() {
            ...
            if (selectedTitle != null && selectedTitle.getWidth() > 0) {
                ...
                int offset = (int) ((right - left) * (1.0f - mSelectedIndicatorWeight) / 2);
                left += offset;
                right -= offset;
                ...
                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    ...
                    int nextOffset = (int) ((nextRight - nextLeft) * (1.0f - mSelectedIndicatorWeight) / 2);
                    nextLeft += nextOffset;
                    nextRight -= nextOffset;
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            int targetOffset = (int) ((targetView.getRight() - targetView.getLeft()) * (1.0f - mSelectedIndicatorWeight) / 2);
            final int targetLeft = targetView.getLeft() + targetOffset;
            final int targetRight = targetView.getRight() - targetOffset;
            final int startLeft;
            ...
            if (Math.abs(position - mSelectedPosition) <= 1) {
                ...
                final int offset = (int) ((targetView.getRight() - targetView.getLeft()) * (1 - mSelectedIndicatorWeight / 2));
                if (position < mSelectedPosition) {
                    // We're going end-to-start
                    if (isRtl) {
                        startLeft = mIndicatorLeft + offset;
                        startRight = mIndicatorRight + offset;
                    } else {
                        startLeft = mIndicatorLeft - offset;
                        startRight = mIndicatorRight - offset;
                    }
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        setIndicatorPosition(
                                ...
                                startRight + Math.round(fraction * (targetRight - startRight)));
                    }
                });
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        ...
        // We return the default layout params so that we don't blow up if we're given a android.support.design.widget.TabItem
     ...
     * necessary calls back to the provided {@link PPDTabLayout} so that the tab position is
     * kept in sync.
     *
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<PPDTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(PPDTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final PPDTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
    ...
    /**
     * A {@link PPDTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements PPDTabLayout.OnTabSelectedListener {
        ...
        @Override
        public void onTabSelected(PPDTabLayout.Tab tab) {
            mViewPager.setCurrentItem(tab.getPosition());
        }

        @Override
        public void onTabUnselected(PPDTabLayout.Tab tab) {
            // No-op
        }

        @Override
        public void onTabReselected(PPDTabLayout.Tab tab) {
            // No-op
        }
    }
